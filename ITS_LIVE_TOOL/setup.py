...
# %% auto 0
__all__ = ['var_ls', 'glacier_hw', 'glacier_pt', 'glacier_centerline', 'point_to_gdf', 'remove_empty_timesteps',
           'Glacier_centerline', 'Glacier_lw', 'Glacier_hw', 'Glacier_Point', 'calc_min_tbaseline']
...
class Glacier_lw():
    ...
    def __init__(self, name, rgi_id, utm_crs):
        ...
        self.outline_prj = self.outline.to_crs(self.utm_zone)
...
class Glacier_hw():
    ...
    '''
    def __init__(self, name, rgi_id, working_dir_path):

        self.name = name
        self.rgi_id = rgi_id
        self.rgi_region = rgi_id.split('-')[1].split('.')[0]
        self.outline = self._download_rgi()
        self.utm_crs = str(self.outline.estimate_utm_crs())
        self.utm_zone = str(self.outline.estimate_utm_crs())
...
glacier_hw = Glacier_hw('shimo', 'RGI60-14.06794', wd_path)

# %% ../nbs/04_setup.ipynb 27
glacier_pt = Glacier_Point(glacier_hw.name, 'mid ish', glacier_hw.rgi_id, [coords[0][1], coords[0][0]], var_ls)

# %% ../nbs/04_setup.ipynb 28
glacier_centerline = Glacier_Centerlie(glacier_hw.name, glacier_hw.rgi_id)
