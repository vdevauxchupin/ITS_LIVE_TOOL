...
# %% auto 0
__all__ = ['urls', 'X_valid', 'Y_valid', 'X_tot', 'Y_tot', 'format_input', 'format_urls', 'get_extents', 'design_matrices',
           'Inv_reg']
...
def format_input(input_coords, input_gpdf, input_urls):

    for element in range(len(input_coords)):

        if len(input_coords) == 1: 
            print('1 point/ glacier selected, output will be a dict')

            input_dict = {'RGIID': input_gpdf.iloc[0]['RGIID'],
                          'coords':input_coords[0],
                          'url': urls[0],
                          'gdf':input_gpdf}
            return input_dict
            ...
            print('no selections on map have been made')

        elif len(input_coords)  > 1: 
            print('multiple points selected, output will be a list of dicts')

            dict_ls = []

            input_dict = {'RGIID': input_gpdf.iloc[element]['RGIID'],
                          'coords':input_coords[element],
                          'url': urls[element],
                          'gdf':input_gpdf}

            dict_ls.append(input_dict)
